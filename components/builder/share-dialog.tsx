...
import { setWorkoutPublic } from '@/lib/actions/workouts'
...
export function ShareDialog({ workoutId, initialIsPublic, initialSlug, label }: ShareDialogProps) {
  const [isPublic, setIsPublic] = useState(initialIsPublic)
  const [slug, setSlug] = useState<string | null>(initialSlug)
  const [pending, setPending] = useState(false)
  const [copied, setCopied] = useState(false)

  const shareUrl = slug && typeof window !== 'undefined' ? `${window.location.origin}/w/${slug}` : null

  async function handleToggle() {
    setPending(true)
    try {
      const { share_slug } = await setWorkoutPublic(workoutId, !isPublic)
      setIsPublic((v) => !v)
      setSlug(share_slug)
    } finally {
      setPending(false)
